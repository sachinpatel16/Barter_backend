...
class MerchantDealViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        """
        Create a new merchant deal with automatic category assignment.
        
        If no category is provided in the request, the merchant's default
        category will be automatically assigned to the deal.
        """
        # Get the merchant profile
        merchant_profile = self.request.user.merchant_profile
        
        # Automatically assign merchant's category to the deal if not provided
        if not serializer.validated_data.get('category') and merchant_profile.category:
            serializer.validated_data['category'] = merchant_profile.category
...
class DealDiscoveryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    filterset_fields = ['points_offered', 'merchant__city']
    ...
    def get_queryset(self):
        ...
        # Handle multiple category IDs filtering
        if category_ids:
            try:
                # Split comma-separated category IDs and convert to integers
                category_id_list = [int(cat_id.strip()) for cat_id in category_ids.split(',') if cat_id.strip()]
                if category_id_list:
                    queryset = queryset.filter(category_id__in=category_id_list)
            except (ValueError, TypeError):
                # If invalid category IDs provided, ignore the filter
                pass
...
class MerchantDealRequestViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        """List deal requests with optional accept/reject actions"""
        ...
        if action and request_id:
            try:
                ...
                deal_request = self.get_queryset().get(id=request_id)
                return self._handle_request_action(deal_request, action, request)
            except (ValueError, MerchantDealRequest.DoesNotExist):
                return Response({'error': 'Invalid request ID'}, status=400)
        
        # Regular list view
        return super().list(request, *args, **kwargs)
    
    def _handle_request_action(self, deal_request, action, request):
        """Handle accept or reject action for a deal request"""
        user = request.user.merchant_profile
        
        # Only deal creator can accept/reject
        if deal_request.deal.merchant != user:
            return Response({'error': 'Only deal creator can accept/reject requests'}, status=403)
        
        if deal_request.status != 'pending':
            return Response({'error': 'Request is not pending'}, status=400)
        
        if action == 'accept':
            return self._accept_request(deal_request)
        elif action == 'reject':
            return self._reject_request(deal_request)
        else:
    ...
    def _accept_request(self, deal_request):
        ...
        # Automatically reject all other pending requests for the same deal
        other_requests = MerchantDealRequest.objects.filter(
            deal=deal_request.deal,
            status='pending'
        ).exclude(id=deal_request.id)
        
        rejected_count = other_requests.count()
        ...
        confirmation = MerchantDealConfirmation.objects.create(
            ...
            points_exchanged=deal_request.deal.points_offered,
            status='confirmed',
    ...
    def _reject_request(self, deal_request):
        ...
        # Send notification to rejected merchant
        MerchantNotification.objects.create(
            merchant=deal_request.requesting_merchant,
            notification_type='deal_rejected',
            title='Deal Request Rejected',
    ...
    def perform_create(self, serializer):
        # For fixed point deals, points_requested must match deal points_offered
        deal = serializer.validated_data['deal']
        serializer.validated_data['points_requested'] = deal.points_offered
        ...
        MerchantNotification.objects.create(
            ...
            message=f'{request_obj.requesting_merchant.business_name} requested your deal "{request_obj.deal.title}" for {request_obj.points_requested} points',
    ...
    def accept(self, request, pk=None):
        ...
        # Create deal confirmation (fixed points - no negotiation)
        confirmation = MerchantDealConfirmation.objects.create(
            deal_request=deal_request,
            deal=deal_request.deal,
            merchant1=deal_request.deal.merchant,
            merchant2=deal_request.requesting_merchant,
            points_exchanged=deal_request.deal.points_offered,  # Fixed points from deal
            status='confirmed',
            confirmation_time=timezone.now()
        )
        
        # Update deal points (use fixed deal points)
        deal = deal_request.deal
        deal.points_used += deal.points_offered  # Use fixed deal points
        deal.save()
...
class MerchantReceivedRequestsViewSet(viewsets.ModelViewSet):
    """
    ViewSet for merchants to see and manage requests they have received for their deals
    """
    serializer_class = MerchantDealRequestSerializer
    permission_classes = [IsAuthenticated]
    filter_backends = (DjangoFilterBackend, SearchFilter)
    ...
    def get_queryset(self):
        ...
        user = self.request.user
        if hasattr(user, 'merchant_profile'):
            # Get requests for deals created by current merchant
            return MerchantDealRequest.objects.filter(deal__merchant=user.merchant_profile)
        return MerchantDealRequest.objects.none()
    ...
    def list(self, request, *args, **kwargs):
        """List received requests with optional accept/reject actions"""
        # Check for action parameters
        action = request.query_params.get('action')
        request_id = request.query_params.get('request_id')
...
class SimpleVisitViewSet(viewsets.ModelViewSet):
    ...
    def track_visit(self, request):
        ...
        return Response({
            ...
            'visit': SimpleVisitSerializer(visit, context={'request': request}).data
        }, status=201)
...
class StoreVoucherViewSet(viewsets.ModelViewSet):
    ...
    def give_voucher(self, request):
        ...
        return Response({
            ...
            'voucher': StoreVoucherSerializer(voucher, context={'request': request}).data
        }, status=201)
