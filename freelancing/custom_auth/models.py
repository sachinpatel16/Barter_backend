...
class MerchantDeal(BaseModel):
    ...
    def __str__(self):
        return f"{self.merchant.business_name} - {self.title} ({self.points_offered} points)"
...
class MerchantDealRequest(BaseModel):
    ...
    points_requested = models.DecimalField(max_digits=12, decimal_places=2)  # Points requested (fixed - no counter offers)
...
class SimpleVisit(BaseModel):
    ...
    class Meta:
        ordering = ['-visit_date']
        unique_together = ['user', 'merchant', 'visit_date']  # One visit per day per merchant
