...
class MerchantDealSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'preferred_categories', 'terms_conditions', 'request_count',
    ...
    def validate(self, data):
        # Expiry date is optional, but if provided, it should be in the future
...
class MerchantDealCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MerchantDeal
        fields = [
            'title', 'description', 'points_offered', 'category',
    ...
    def validate(self, data):
        """Auto-calculate deal_value from points_offered (10 points = 1 rupee)"""
        if 'points_offered' in data:
            # Auto-calculate deal_value: 10 points = 1 rupee
            data['deal_value'] = data['points_offered'] / 10
        return data
...
class MerchantDealRequestSerializer(serializers.ModelSerializer):
    ...
    requesting_merchant_logo = serializers.SerializerMethodField()
    deal_title = serializers.CharField(source='deal.title', read_only=True)
    deal_merchant = serializers.CharField(source='deal.merchant.business_name', read_only=True)
    deal_merchant_logo = serializers.SerializerMethodField()
    ...
    def get_requesting_merchant_logo(self, obj):
        if obj.requesting_merchant.logo:
            request = self.context.get('request')
            if request:
                return request.build_absolute_uri(obj.requesting_merchant.logo.url)
            return obj.requesting_merchant.logo.url
        return None
    
    def get_deal_merchant_logo(self, obj):
        if obj.deal.merchant.logo:
            request = self.context.get('request')
            if request:
                return request.build_absolute_uri(obj.deal.merchant.logo.url)
            return obj.deal.merchant.logo.url
        return None
    ...
    def validate(self, data):
        ...
        # Fixed point deals - points requested must match deal points offered
        if data['points_requested'] != data['deal'].points_offered:
            raise serializers.ValidationError(
                f"Points requested must match deal points offered ({data['deal'].points_offered})"
            )
...
class MerchantDealConfirmationSerializer(serializers.ModelSerializer):
    ...
    def get_merchant1_logo(self, obj):
        if obj.merchant1.logo:
            request = self.context.get('request')
            if request:
                return request.build_absolute_uri(obj.merchant1.logo.url)
            return obj.merchant1.logo.url
        return None
    ...
    def get_merchant2_logo(self, obj):
        if obj.merchant2.logo:
            request = self.context.get('request')
            if request:
                return request.build_absolute_uri(obj.merchant2.logo.url)
            return obj.merchant2.logo.url
        return None
...
class MerchantNotificationSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'notification_type', 'title', 'message', 'deal', 'confirmation',
...
class MerchantPointsTransferSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'confirmation', 'from_merchant', 'from_merchant_name', 'to_merchant',
...
class SimpleVisitSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SimpleVisit
        fields = [
            'id', 'user', 'user_name', 'merchant', 'merchant_name', 'merchant_logo',
            'visit_date', 'notes', 'create_time'
        ]
        read_only_fields = ['id', 'user_name', 'merchant_name', 'merchant_logo', 'create_time']
...
class StoreVoucherSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StoreVoucher
        fields = [
            'id', 'voucher_code', 'title', 'description', 'merchant', 'merchant_name', 'merchant_logo',
        ...
        read_only_fields = ['id', 'voucher_code', 'merchant_name', 'merchant_logo', 'user_name', 'used_at', 'used_amount', 'create_time']
