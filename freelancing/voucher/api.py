...
class VoucherViewSet(viewsets.ModelViewSet):
    ...
    def share_gift_card(self, request, pk=None):
        ...
        try:
            ...
            try:
                # First check if user is the merchant who created this gift card
                if voucher.merchant.user == request.user:
                    # Merchant sharing their own gift card - create a dummy redemption record if not exists
                    user_redemption, created = UserVoucherRedemption.objects.get_or_create(
                        user=request.user,
                        voucher=voucher,
                        is_active=True,
                        defaults={
                            'purchase_cost': Decimal('0.00'),
                            'purchase_reference': f"MERCHANT-{voucher.id}-{timezone.now().strftime('%Y%m%d%H%M%S')}",
                            'purchase_status': 'purchased',
                            'is_gift_voucher': True,
                            'wallet_transaction_id': f"MERCHANT-GIFT-{voucher.id}-{timezone.now().strftime('%Y%m%d%H%M%S')}",
                            'voucher_purchase_count': 1,
                            'max_redemption_allowed': 1
                        }
                    )
                else:
                    # Regular user trying to share - must have purchased it
...
# Send WhatsApp message
    ...
    def _format_phone_for_whatsapp(self, phone_number):
        ...
        try:
            ...
            print(f"Invalid phone number format for WhatsApp: {phone_number} -> {clean_phone}")
            return None
...
class WhatsAppContactViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=["post"], url_path="test-whatsapp-send")
    def test_whatsapp_send(self, request):
        """Test WhatsApp message sending using Twilio"""
        try:
            phone_number = request.data.get('phone_number')
            test_message = request.data.get('message', 'Test message from Bartr')
